...
class StatisticsApiController extends AbstractController
{
    ...
    public function getInvoicesGenerationStatisticsAndAmounts(Request $request, PaginatorInterface $paginator): JsonResponse
    {
        ...
        foreach ($statistics as $result) {
            ...
            switch ($yearMonth) {
                case '01' :
                    $result['monthYear'] = 'Janvier' . substr($result['date'], 2, 5);
                    $month = "01";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '02' :
                    $result['monthYear'] = 'Février' . substr($result['monthYear'], 2, 5);
                    $month = "02";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '03' :
                    $result['monthYear'] = 'Mars' . substr($result['monthYear'], 2, 5);
                    $month = "03";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '04' :
                    $result['monthYear'] = 'Avril' . substr($result['monthYear'], 2, 5);
                    $month = "04";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '05' :
                    $result['monthYear'] = 'Mai' . substr($result['monthYear'], 2, 5);
                    $month = "05";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '06' :
                    $result['monthYear'] = 'Juin' . substr($result['monthYear'], 2, 5);
                    $month = "06";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '07' :
                    $result['monthYear'] = 'Juillet' . substr($result['monthYear'], 2, 5);
                    $month = "07";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '08' :
                    $result['monthYear'] = 'Août' . substr($result['monthYear'], 2, 5);
                    $month = "08";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '09' :
                    $result['monthYear'] = 'Septembre' . substr($result['monthYear'], 2, 5);
                    $month = "09";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '10' :
                    $result['monthYear'] = 'Octobre' . substr($result['monthYear'], 2, 5);
                    $month = "10";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '11' :
                    $result['monthYear'] = 'Novembre' . substr($result['monthYear'], 2, 5);
                    $month = "11";
                    $year = substr($auxVar, 3, 4);
                    break;
                case '12' :
                    $result['monthYear'] = 'Décembre' . substr($result['monthYear'], 2, 5);
                    $month = "12";
                    $year = substr($auxVar, 3, 4);
                    break;
